...
search_name = input("Enter the partial name of the employee: ").strip().lower()

# Define what to search for (phone or email)
search_type = input("Do you want to search for 'phone' or 'email'? ").strip().lower()

if search_type == "phone":
    results = [
        (employee[1], employee[3])
        for employee in employees
        if search_name in employee[1].lower()
    ]
